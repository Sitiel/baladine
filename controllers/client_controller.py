...
elle peux aussi servir a se reconnecter lorsqu'on a ete deconnecte (delai de 36h dans le jeu)
"""


def join_game(playerJoinUsername):
    # Test pour savoir si UserName est deja utilise en ce moment
    name = slugify(playerJoinUsername['name'], max_length=15)
    ...
    if jExist is None:
        ...
        z = zone(longitude, latitude, rayon, "stand")
        ...
        if name not in json_model.lastInfoFromPlayer:
            json_model.lastInfoFromPlayer[name] = 0
        ...
        if json_model.currentHour - json_model.lastInfoFromPlayer[name] >= 36:
            ...
            location = \
                {
                    "longitude": joueurStand.zone_posX,
                    "latitude": joueurStand.zone_posY
                }
            drinksOffered = []
...
def map_player_name_get(playerName):
    ...
    region = \
        {
            ...
            "span":
                  {
                      "latitudeSpan": c.carte_longueur,
                      "longitudeSpan": c.carte_largeur
                  }
        }
    ...
    for e_joueur in r:
        ...
        for zone in zones:
            zone_location = \
                {
                    "latitude": zone.zone_posY,
                    "longitude": zone.zone_posX
                }
            prop.append(
                {
                    "kind": zone.zone_type,
    ...
    total = {
        'availablesIngredient': [i.toJson() for i in ingredients],
        'map': final_map,
        "playerInfo": info
    }
